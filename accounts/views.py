...
def index(request):
    ...
    alapprove = Leave.objects.filter(
        appmanager=current_user).filter(status='Unactioned')
    """user personal information """
    myhours = Establishment.objects.filter(user=current_user).order_by('day')
    hours_total = Establishment.objects.filter(
        user=current_user).aggregate(Sum('hours')).get('hours__sum', 0.00)
    ...
    absence = Absence.objects.filter(
        absence_end__isnull=True).filter(user__profile__team=teaml)
    """overtime total for cicle"""
    myottotal = Overtime.objects.filter(user=current_user).exclude(
